 ...
 */
class AuthUserRole extends BaseController {
  constructor(req, res) {
    // Prepare data for parent object
    const searchFields = ["user_id", "role_id"]
    // Call constructor of parent object, passing in this component name, database, and table
    super(req, res, "AuthUserRole", "g3tools", "auth_user_role", searchFields)
    // Database fields to be inserted/updated
    this.primaryFields = ["user_id", "role_id"]
    this.fields = []
    this.req = req
    this.res = res
  }
  ...
  async parseReq(method) {
    const primaryData = [this.req.body.userId, this.req.body.roleId]
    const fieldData = []
...
module.exports = { AuthUserRole, validate }
