const { itemMasterDb } = require("../3-db/item-master.db")
 ...
 */
exports.itemsGetAll = async (page, limit, orderBy, orderDir) => {
  try {
    // Convert page/limit to limit/offset for mySql
    const pagination = utils.convertPagination(page, limit)
    // Get db results
    const dbResult = await itemMasterDb.itemsGetAll(
      pagination.limit,
      pagination.offset,
      orderBy,
      orderDir
...
exports.itemsFindSome = async (page, limit, searchText, orderBy, orderDir) => {
  try {
    ...
    // Get db results
    const dbResult = await itemMasterDb.itemsFindSome(
      pagination.limit,
      pagination.offset,
      searchText,
      orderBy,
...
exports.tableValidation = async data => {
  ...
  valError = await utils.primaryExists(
    "Item Number",
    itemNumber,
    "g3tools",
    "item_master",
